...
| Tile size in memory | 24 bytes | Three 8-byte bitplanes per 8x8 character. |
...
| Nametables | 8 tables, 40x25 bytes each | Background tile maps. Two 2x2 sets support four-way scrolling plus optional staging or page flips. |
| Attribute tables | 8 tables, 40x25 bytes each | Background cell attribute maps: palette, flip, priority, and alternate character bank selection. |
...
| OAM size | 256 sprites * 5 bytes = 1280 bytes | OAM stores the sprite list: position, character id, attributes, and extended coordinates. |
...
- **Character plane:** One bitplane inside a character bank. Plane 0 stores the
low bit for every pixel, plane 1 stores the middle bit, and plane 2 stores the
high bit.
...
For background tiles, each cell in the attribute table selects one of these 16
palette banks and controls per-cell tile attributes. For sprites, each sprite
attribute byte selects one palette bank.
...
Each attribute table matches a nametable cell-for-cell:
...
| `7` | `CHR_ALT` | `0` = use `bg_chr_bank`, `1` = use `bg_chr_bank_alt`. |

`FLIP_X` and `FLIP_Y` reduce duplicate character art for mirrored slopes,
corners, borders, decorations, and UI pieces.

`PRIORITY` lets a background cell act like a foreground object. The renderer
still draws the cell as part of the background, but its nonzero pixels hide
sprite pixels at the same screen positions. This is useful for scenery that
should appear in front of sprites, such as pillars, trees, door frames, railings,
or UI frames. This is separate from sprite priority: sprite priority moves a
whole sprite behind all nonzero background pixels, while cell priority marks
specific background cells as foreground.

`CHR_ALT` gives each background cell one extra character-bank selection bit. The
nametable byte still selects a character index from `0` to `255`; `CHR_ALT`
chooses whether that index is read from `bg_chr_bank` or `bg_chr_bank_alt`.
This allows a background to mix two 256-character sets without widening the
nametable.
...
| Bits | Name | Description |
| - | - | - |
| `0-1` | `X_HI` | Bits 8-9 of a signed 10-bit X coordinate. |
| `2` | `Y_HI` | Bit 8 of a signed 9-bit Y coordinate. |
...
| `0x0C200` | `0x1F40` | `0x0E13F` | 8 nametables |
| `0x0E140` | `0x1F40` | `0x1007F` | 8 attribute tables |
| `0x10080` | `0x0500` | `0x1057F` | Sprite OAM |
| `0x10580` | `0x0100` | `0x1067F` | Dirty flags and generation counters |
| `0x10680` | `0x0980` | `0x10FFF` | Reserved video expansion |
...
| `48-55` | Nametables 0-7 | `0x0C200 + table * 1000` | `1000` |
| `56-63` | Attribute tables 0-7 | `0x0E140 + table * 1000` | `1000` |
| `64` | Sprite OAM | `0x10080` | `1280` |
| `65` | Video control block | `0x00000` | `256` |
| `66` | Dirty flags | `0x10580` | `256` |
| `67-79` | Reserved video indexes | varies | varies |
...
| `0x0A` | 1 | `active_set` | `0` = tables 0-3, `1` = tables 4-7. |
| `0x0B` | 1 | `viewport_mode` | `0` single table, `1` horizontal pair, `2` vertical pair, `3` 2x2 four-way plane. |
| `0x0C` | 2 | `scroll_x` | Pixel scroll X within the active background plane. |
| `0x0E` | 2 | `scroll_y` | Pixel scroll Y within the active background plane. |
| `0x10` | 1 | `bg_chr_bank` | Primary character bank used for background tiles. |
...
| `0x18` | 1 | `bg_chr_bank_alt` | Alternate background character bank used when cell attribute `CHR_ALT` is set. |
| `0x19` | 7 | `reserved` | Reserved for timing and transport metadata. |
...
```text
4 nametables        = 4000 bytes
4 attribute tables = 4000 bytes
OAM                 = 1280 bytes
control block       = 256 bytes
total               = 9536 bytes, about 9.3 KiB per frame
```

At 25 FPS this is about 233 KiB/s. At 30 FPS this is about 279 KiB/s. That is
...
| Character banks | 1 character across 3 planes, 24 bytes total |
| Nametables | range of cells or 40-byte row |
| Attribute tables | range of cells or 40-byte row |
...
2. Render background tiles if `ENABLE_BG` is set, applying each cell's palette,
   flip, character-bank, and priority attributes.
3. Render sprites with transparency if `ENABLE_SPRITES` is set. Sprite pixels
   should be hidden behind nonzero background pixels when either the sprite's
   `PRIORITY` bit or the background cell's `PRIORITY` bit requests it.
...
| Data | Bytes |
| - | - |
| Four nametables | 4000 |
| Four attribute tables | 4000 |
| OAM | 1280 |
| Control block | 256 |
| Total payload | 9536 |
...
| Palette banks | 256 |
| Nametables | 8000 |
| Attribute tables | 8000 |
| OAM | 1280 |
| Control block | 256 |
| Total payload | 66944 |

Even a full snapshot is only about 65 KiB. Sending that on connect or as an
...
- Preconfigure indexes 16-23, 32-47, 48-55, 56-63, 64, 65, and 66.
